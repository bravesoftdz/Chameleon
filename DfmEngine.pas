...
var
  k : TBorderIcon;
begin
  if ABorderIcons = [] then begin
    Result := '[]';
    Exit;
  end;
  Result := '[';
  for k:= biSystemMenu to biHelp do
    if k in ABorderIcons then Result := Result + biStr[k] + ',';
...
begin
  bi := [];
  if BitTest(style, WS_SYSMENU) then Include(bi, biSystemMenu);
  if BitTest(style, WS_MINIMIZEBOX) then Include(bi, biMinimize);
  if BitTest(style, WS_MAXIMIZEBOX) then Include(bi, biMaximize);
  if BitTest(exstyle, WS_EX_CONTEXTHELP) then Include(bi, biHelp);
...
begin
  ...
  try
    ...
    if BelongsToPicture then Memory.WriteString('TBitmap');
    size := 0;
...
begin
  ...
  try
    ...
    if BelongsToPicture then k.WriteString('TIcon');
...
begin
  ...
  if StrIComp(class_name, 'Static') = 0 then
    ...
    begin
      ...
      else if BitTest(style, BS_CHECKBOX) or BitTest(style, BS_AUTOCHECKBOX) or BitTest(style, BS_3STATE) or BitTest(style, BS_AUTO3STATE) then
        CtlType := kcCheckBox
      else
        CtlType := kcButton;
    end
